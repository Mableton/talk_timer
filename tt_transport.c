...
#define TT_BT_KEYS_PATH APP_DATA_PATH(".bt_hid.keys")

/* Eigener Gerätename ("Talk <Flippername>") und eigene MAC, damit der Rechner den
 * Talk Timer weder mit dem BT-Remote der Firmware noch mit Zoom Remote
 * (mac_xor 0x0005) verwechselt */
static BleProfileHidParams tt_ble_params = {
    .device_name_prefix = "Talk",
    .mac_xor = 0x0006,
};

/* ---- USB ----------------------------------------------------------------- */

static void tt_transport_usb_start(TtTransport* transport) {
...
static void tt_transport_usb_stop(TtTransport* transport) {
...
static void tt_transport_ble_status_callback(BtStatus status, void* context) {
    TtTransport* transport = context;
    transport->ble_connected = (status == BtStatusConnected);
}

static void tt_transport_ble_start(TtTransport* transport) {
    ...
    storage_simply_mkdir(storage, EXT_PATH("apps_data/talk_timer"));
    furi_record_close(RECORD_STORAGE);
    bt_keys_storage_set_storage_path(transport->bt, TT_BT_KEYS_PATH);

    transport->ble_profile = bt_profile_start(transport->bt, ble_profile_hid, &tt_ble_params);
    furi_check(transport->ble_profile);

    furi_hal_bt_start_advertising();
    bt_set_status_changed_callback(transport->bt, tt_transport_ble_status_callback, transport);
    FURI_LOG_I(TAG, "Bluetooth-HID gestartet");
}

static void tt_transport_ble_stop(TtTransport* transport) {
...
void tt_transport_forget_bt_pairing(void) {
    ...
    Storage* storage = furi_record_open(RECORD_STORAGE);
    storage_simply_remove(storage, TT_BT_KEYS_PATH);
    furi_record_close(RECORD_STORAGE);
}
...
TtTransport* tt_transport_alloc(TtConn conn) {
    TtTransport* transport = malloc(sizeof(TtTransport));
    transport->conn = conn;
    ...
    if(conn == TtConnUsb) {
        tt_transport_usb_start(transport);
    } else {
        tt_transport_ble_start(transport);
    }
    return transport;
}
...
void tt_transport_free(TtTransport* transport) {
    ...
    if(transport->conn == TtConnUsb) {
        tt_transport_usb_stop(transport);
    } else {
        tt_transport_ble_stop(transport);
    }
    free(transport);
}
...
bool tt_transport_is_connected(TtTransport* transport) {
    furi_assert(transport);
    if(transport->conn == TtConnUsb) return furi_hal_hid_is_connected();
    return transport->ble_connected;
}

bool tt_transport_press(TtTransport* transport, uint16_t hid_code) {
    furi_assert(transport);
    if(transport->conn == TtConnUsb) return furi_hal_hid_kb_press(hid_code);
...
bool tt_transport_release(TtTransport* transport, uint16_t hid_code) {
    furi_assert(transport);
    if(transport->conn == TtConnUsb) return furi_hal_hid_kb_release(hid_code);
